...
- `ip_resolver` (string) — When getting the VM's IP address after it's cloned, use this IP resolving strategy to determine the IP. By default uses the default method `tart ip` uses.
